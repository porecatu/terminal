...
| Token | Valor | Uso |
|---|---|---|
| Acento | `#5ed3bc` | logotipo, foco de input, nome do shell na status, indicador de commits atrás do remoto (§2.8), prompt da paleta |
...
| Zona | Segmento | Cor |
|---|---|---|
| Esquerda | nome do shell | `#5ed3bc` (acento, §1.5) |
...
**O segmento de commits atrás do remoto** ([ADR-0052](../adr/0052-sincronizacao-com-o-remoto-do-git.md), [PRD-013](../prd/prd-013-sincronizacao-com-o-remoto-do-git.md)) vem logo depois da branch: a contagem é propriedade dela, como a branch é propriedade do diretório. Ícone de seta para baixo da mesma face Lucide, com a **mesma em** do ícone de branch, e o rótulo por extenso — `3 commits atrás`, com singular e plural. Entre o ícone e o texto não há `gap`, pela mesma razão do segmento de branch. Ele é o **segundo item colorido da barra** e o **primeiro alvo clicável** dela, e as duas coisas andam juntas: sob o cursor ganha **sublinhado** e o cursor vira o de mão, a mesma affordance do hyperlink OSC 8 (§3). Com commits locais ainda não enviados, o rótulo vira `2 atrás, 1 à frente` e o segmento **deixa de ser clicável** — o `pull --ff-only` falharia por definição, e a barra não oferece um alvo que já se sabe quebrado. **Sem commits novos o segmento inteiro desaparece**, como o de branch fora de um repositório: não há versão apagada nem "0 atrás". Nunca é truncado — em janela estreita cai inteiro, como a branch.
...
Fica **dentro do quadro arredondado do terminal** (§2.7), encostada no topo, ocupando a largura interna dele. Altura **46**: o campo de texto (item 1) mantém os `input_height` cheios do editor (30), com **8px** de respiro acima e abaixo dele até a borda da barra — sem isso o campo, única caixa com fundo/borda própria da barra além do trilho do toggle, esticava pela altura inteira e encostava nas bordas sem margem nenhuma, enquanto tinha `padding_left` de sobra à esquerda. Fundo `#1a1e25` e borda inferior `1px #2e343e`, os do aviso e do popover. Os cantos superiores acompanham o raio do quadro; os inferiores são retos. **Sem sombra**: ela é encostada e opaca, não flutua sobre o terminal como os cinco widgets da lista do ADR-0032 §2.
...
| Nada — o canvas nunca desenhou nada de Git, e o PRD-009 listava `git` inteiro em "Fora de escopo" | **Ícone `git-branch` mais o nome da branch**, na zona esquerda, depois do diretório. A palavra `git` no PRD agrupava duas coisas de custo muito diferente: ler qual é a branch são 30 bytes revalidados por `mtime`, e saber se a árvore está suja é um `git status` completo — a primeira entrou, a segunda continua fora e é fronteira, não pendência. **Zero valor novo**: o ícone sai da face Lucide já embutida (que não é subsetada, então nenhum recorte precisou ser refeito) e a em dele é `icon_em_size * 0.8`, o multiplicador que o botão de configurações já usava. Entre ícone e nome não há `gap` — a sobra do avanço de 1 em sobre o desenho de 0.84 em é a folga. Pedido do dono do produto depois de a barra estar em uso | [ADR-0049](../adr/0049-branch-git-na-barra-de-status.md), fora de fase |
| Nada — e a §2.8 dizia, em letra fixa, que a barra é **"sem alvo clicável"**, além de o ADR-0048 §3 declarar o nome do shell **"o único item colorido"** | **Indicador de commits atrás do remoto**, clicável, depois da branch. Quebra as duas linhas acima de uma vez, e as duas foram decisão do dono do produto sobre alternativas mostradas: cor **sempre no acento** `#5ed3bc` (recusadas a cor de base com acento só no hover, e o texto sublinhado sem ícone), com **sublinhado sob o cursor** e o cursor de mão — a mesma affordance do hyperlink OSC 8, que é o vocabulário que o app já tem para "isto é clicável". O segundo acento não apaga a distinção que o shell tinha porque o segmento **quase nunca está presente**: sem commits novos ele não existe, e o caso comum da barra continua com um item colorido só. Rótulo por extenso (`3 commits atrás`), recusado o `↓ 3` por economia e o `↓ 3 · atualizar` por largura. **Zero cor nova** — o acento já era token com outros consumidores; **um ícone novo**, da face Lucide não subsetada. Sobre os 6px de resize: o retângulo do indicador vence dentro dos próprios limites e o resto da faixa continua resize, que é literalmente a saída que o ADR-0048 já tinha escrito na alternativa recusada | [ADR-0052](../adr/0052-sincronizacao-com-o-remoto-do-git.md), fora de fase |
