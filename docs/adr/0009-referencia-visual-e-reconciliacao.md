...
**Status:** Superseded by [ADR-0027](0027-controles-de-janela-e-resize-proprios.md), [ADR-0028](0028-o-binario-como-referencia-visual.md) e [ADR-0032](0032-interface-do-v1-fechada.md), os três **parcialmente**:
...
- **ADR-0032** — a seção 5 ("Indicador de grupo combinável") por inteiro. `indicator_style` deixa de ser lista combinável e deixa de existir: o indicador do v1 é a pílula mais a cápsula, e `underline`, `left-bar` e `outline` saem de escopo.

As seções 2, 3, 6, 7 e 8 seguem em vigor, com a ressalva do ADR-0027 acima.
**Data:** 2026-08-26
...
> **Nota de fato, 2026-09-15 — a aposta desta seção foi cobrada duas vezes, e pagou as duas.** Dois dos seis elementos saíram da tabela acima, pelo mecanismo que ela mesma previu: a **barra de status** em 2026-09-09 ([PRD-009](../prd/prd-009-barra-de-status.md) promovido, [ADR-0048](0048-barra-de-status.md)) e os **painéis divididos** em 2026-09-15 ([PRD-006](../prd/prd-006-paineis-divididos.md) promovido, [ADR-0053](0053-paineis-divididos.md)) — os dois fora da ordem de fases, com o rascunho servindo exatamente para o que foi escrito. O caso dos painéis cobra a frase do parágrafo seguinte de forma literal: *"saber que existirão painéis muda se `Tab` é um alias de terminal ou uma struct própria"* — e foi por `Tab` já ser struct própria que a árvore de painéis coube sem revogar decisão nenhuma. **A decisão desta seção não muda**: elemento `[v2]` continua sendo desenho aprovado e não escopo, e o que sai da lista sai por PRD promovido mais ADR próprio, nunca por commit. Um pedaço do desenho dos painéis **não** saiu e não vai sair: o **cabeçalho de painel** foi recusado pelo ADR-0053, e é o primeiro elemento do canvas classificado como decisão de não fazer em vez de fase futura.
