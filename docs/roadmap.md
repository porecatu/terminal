...
| F6 — Polimento | **aberta** (PRD-011 e ADR-0041 a 0044 escritos); implementação não iniciada |
...
1. **Busca no motor e no modelo.** `porecatu-term` encapsula o `RegexSearch` do `alacritty_terminal` num módulo próprio (`search.rs`), sem deixar tipo do motor atravessar a fronteira, e devolve ocorrências como ranges de posição na grade; o `GridSnapshot` passa a carregá-las. Tudo função pura, testável sem GPU e sem janela — mesmo padrão da etapa 1 da F4 e da F5. É aqui que se mede o teto do custo de busca num scrollback cheio, **antes** de existir barra.
