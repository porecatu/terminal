...
| F6 — Polimento | **fechada** (dívida de verificação e uma métrica fora do alvo, ver seção dela) |
...
- [**ADR-0044**](adr/0044-empacotamento-e-release.md) — instalador nativo por plataforma, sem assinatura de código. Traz também três correções que são a mesma conversa: `x86_64-apple-darwin` entra na matriz, `--locked` passa a valer no `ci.yml` (a última pendência aberta da F0) e a **atribuição das fontes embutidas passa a acompanhar o artefato** — hoje o `release.yml` copia só `LICENSE` e `README.md`, e publicar assim seria descumprir a OFL e a ISC.
- [**ADR-0045**](adr/0045-primeira-versao-0-7-0.md) — revisita o §3 do ADR-0044 na etapa 6 (o resto do ADR-0044 continua de pé): a primeira release é **`0.7.0`**, não `1.0.0` — decisão do dono do produto ao fechar a fase, não questão técnica: dívida de verificação real (macOS/Linux sem instalação de verdade, leitor de tela confirmado só via UI Automation), bugs conhecidos deixados para depois e melhorias que ele ainda quer fazer antes do compromisso de compatibilidade que `1.0.0` faria.
...
2. **Barra de busca na UI — fechada.** `porecatu-ui` ganhou `search_bar.rs`: `SearchBarState` (termo, alternador de regex, `SearchJob` da etapa 1, ocorrência ativa, erro de padrão que **não** descarta o último resultado válido, RF-11.4), geometria pura (`layout_search_bar`), pintura (`paint_search_bar`) e hit-test (`search_bar_hit`) — mesmo padrão de `overlay.rs`/`group_editor.rs`, testável sem GPU e sem janela. Widget na anatomia exata da §2.21 (já escrita na abertura da fase): campo de texto reusado do editor de grupo (§2.10, com a seleção do ADR-0035), contador, alternador de regex — primeira vez que esse token da §1.5 desenha no v1 — e os três botões de ícone já recortados; zero valor e zero ícone novo, `Layer::ORDER` continua com cinco. Captura de teclado **parcial** (ADR-0041 §3): `handle_search_key` reivindica só texto imprimível, `Enter`/`Shift+Enter`/`Esc` e as teclas de edição do ADR-0035 (`apply_text_field_key`, mais `Backspace`, que a categoria da §3 cobre mas a função não trata — mesma nota de `handle_rename_key`); todo o resto cai pro passo 2, então `F3` continua chegando ao despacho normal e trocar de aba/fechar a janela seguem funcionando com a busca aberta. Realce das ocorrências: `porecatu-ui` corta `Occurrence` (posição absoluta) pela vista com a mesma fórmula de `point_to_viewport` do motor (`linha + scroll_offset`), sem tipo do `alacritty_terminal` atravessar a fronteira, e `paint.rs` resolve a cor — ocorrência não ativa reusa `selection_background`/`foreground` (mesma cor da seleção de texto, por isso abrir a busca a limpa), a ativa usa `cursor` (`#5ed3bc`) e o escuro `#12151a` que a pílula de grupo já usa sobre cor cheia. A busca varre em lotes (`SearchJob::step`, etapa 1) um por frame, sem travar a UI e sem o relógio de animação do ADR-0022 (que a §9 fecha em dois consumidores): pede outro redraw enquanto não terminar, mesmo padrão de "saída do PTY continua chegando em frames seguintes". `search.next`/`search.prev` (e `Enter`/`Shift+Enter`, sem binding) rolam até a ocorrência ativa reservando a altura da barra (`reserved_rows`/`scroll_delta_to_reveal`, ADR-0041 §1) e chamam `Workspace::activate_tab`, que já expande grupo colapsado — fecha o RF-2.17 pela segunda fonte que ele sempre citou. `selection.select_all` (RF-11.16, menu do terminal da etapa 3) ganhou o mecanismo em `porecatu-term` (`Terminal::select_all`, mesmo `Selection` de `start_selection`/`update_selection`) e o despacho, junto com `scrollback.to_top`/`to_bottom` (RF-11.30 — já tinham default embutido e entravam no mapa resolvido; faltava só `Shift+Home`/`Shift+End` em `input.rs`, que hoje resolve scrollback por fora do mapa, mesmo caminho de `Shift+PageUp`/`PageDown`). As quatro ações saíram do `#` comentado no `porecatu.example.toml` e ganharam despacho na mesma leva (ADR-0041 §10: nunca antes dele). Testes: `porecatu-ui` 261→270 unitários (9 novos, `search_bar.rs`, função pura); `porecatu-config` sem teste novo, só a contagem de defaults embutidos ajustada (28→32 comuns, 29→33 macOS) e `example_toml.rs`/`verify-docs.py` continuam batendo.

   **Verificação por gesto, e a dívida que ela deixa.** `cargo run` abre e a janela fica estável — testado isolando o PID do processo lançado e encerrando só ele, nunca por nome (instrução de segurança do usuário, válida daqui em diante). Mouse sintético atravessa a proteção de foco do Windows (achado da F4) e teria verificado os três botões e o alternador sem pedir gesto ao usuário — mas abrir a busca depende de `Ctrl+Shift+F`, teclado sintético não atravessa essa proteção, e esta sessão não tinha ferramenta de automação de desktop (só a de navegador). Sem abrir a barra, não há o que clicar nela. **Dívida de verificação registrada**: abrir a busca e clicar nos três botões/alternador com gesto real fica para quando houver sessão com automação de teclado ou verificação manual do usuário.
3. **Hyperlinks OSC 8 e menu de contexto do terminal — fechada.** `porecatu-term` ganhou `HyperlinkSpan` no `GridSnapshot` (linha, coluna inicial/final do trecho contíguo, par de offsets do id e da uri numa arena `String` reusada — mesmo padrão de `clusters`/`selection`, ADR-0042 §1): o motor **já** entrega `Cell::hyperlink()` pelo `Handler` (ao contrário do OSC 7), então `TermEngine::snapshot_into` monta os spans sozinho, na mesma passada que já monta `cells` — sem o segundo parser que o OSC 7 precisou, e sem UI ter que cortar nada pela vista (diferente de `OccurrenceSpan`, que precisa da etapa 2 para isso). Span nunca atravessa quebra de linha (fechado à força em `col == 0`); dois trechos do mesmo id (`Cell::hyperlink().id()`) ficam amarrados por comparação de conteúdo na arena, não por um id numérico próprio — é o que faz passar o cursor por um link partido em duas linhas sublinhar as duas. `Cell` não mudou um byte. Testes: `porecatu-term` 22→26 (golden-style em `vt_sequences.rs`: sem link, span contíguo com uri recuperável, id amarrando dois trechos partidos por quebra de linha, dois links adjacentes não se fundem).

   `porecatu-ui` ganhou `hyperlink.rs`: `classify_scheme` é pura (sem I/O), normaliza minúsculas/`%`-encoding/espaço nas pontas **antes** de comparar contra a allowlist fechada — é o teste que a mitigação do risco do ADR pede, e passa mesmo com um URI carregando metacaractere de shell no corpo (`$(rm -rf ~)`), provando que só o prefixo até o primeiro `:` é examinado e o resto atravessa intocado até `opener::open`, que recebe a string inteira como **argumento**, nunca concatenada numa linha de shell. `file://` reusa `porecatu_term::parse_file_uri` (tornado `pub`, reexportado) em vez de duplicar a conversão — é a mesma letra de unidade do Windows que o OSC 7 já corrigiu na F5, e duplicar teria reintroduzido a classe de bug que o comentário dela documenta. O crate de abertura é o **`opener` 0.8.5** (`MIT OR Apache-2.0`, compatível com GPLv3): `open`/`reveal` atrás de features distintas (`reveal` habilitada), ambos recebendo o caminho como argumento — nenhuma interpolação de shell em lugar nenhum. `porecatu-ui` decide sozinha o que abrir (§7): `porecatu-term` só reporta span e uri. Testes: 13 novos em `hyperlink.rs` (normalização de maiúsculas/`%`-encoding/espaço, allowlist recusando `javascript:`/`ms-msdt:`, `file` caindo em revelar não em abrir, metacaractere de shell, span-sharing por id, uri lookup).

   **Bug achado e corrigido no caminho: `CellFlags::UNDERLINE` nunca tinha primitiva de desenho.** O ADR-0042 §3 afirmava "o sublinhado é a flag `UNDERLINE` que o snapshot já carrega e o pintor já desenha" — falso: a flag era rastreada (`engine.rs` já convertia SGR 4 nela, com teste próprio) mas `paint.rs` nunca tinha uma primitiva para ela, então sublinhado real de programa (`\e[4m`) nunca apareceu em tela desde a F1. Sem essa primitiva a affordance do RF-11.11 não tinha o que desenhar, então corrigir era pré-requisito, não desvio de escopo. `paint_row_underlines` desenha um traço por trecho contíguo de mesma cor — `CellFlags::UNDERLINE` (bug antigo, agora fechado) **ou** a affordance de hover sob o modificador (spans do mesmo id que `hyperlink::spans_sharing_id_at` resolve a partir da célula sob o cursor) —, na cor do próprio texto e com a espessura de `[terminal.cursor] width` (o mesmo traço do cursor beam/underline, `cursor_primitive`): **zero valor de aparência novo**, `scripts/verify-docs.py` continua batendo os 27 valores.

   O terceiro menu (`terminal_menu.rs`, `overlay.rs`) reusa a anatomia da §2.16 (mesmos tokens de `context_menu.rs`/`group_menu.rs`) — a única diferença estrutural dos outros dois é a **contagem** de itens variar (quatro sem link sob o clique, seis com ele), então `TerminalContextMenu` não indexa uma constante: `terminal_menu_context_items` recomputa a lista de estado ao vivo (seleção corrente, célula sob `menu.anchor`) em toda abertura/navegação/pintura, mesmo padrão de `group_menu::group_action_items`. Copiar/colar/selecionar tudo/abrir a busca chamam exatamente as mesmas funções que a tecla usa — `input::clipboard_copy`/`clipboard_paste` foram fatoradas para fora do `match` de `Ctrl+Shift+C`/`V`, e `selection.select_all`/`search.open` chamam o mesmo caminho de `Action::SelectionSelectAll`/`SearchOpen` — nunca uma segunda implementação (lição da F3, agora também documentada aqui). "Copiar" nasce esmaecido sem seleção, nunca ausente (RF-11.15); os dois itens de link (abrir, copiar) só existem quando o clique caiu sobre um. O clique secundário só abre o menu quando o terminal não faria nada com ele mesmo: programa com mouse reporting pedido (e sem `Shift`) continua ganhando a prioridade de sempre do ADR-0013, a mesma regra que `input::handle_mouse_button` já aplicava — o comentário `"botão direito fica pro menu de contexto, que é F2+"`, ali desde a F1, era literalmente este dia. `Ctrl`+clique (`Cmd` no macOS) sobre um link é a exceção deliberada: vence a prioridade do programa sempre, mesma simetria que o próprio `Shift` já tem para seleção — sem isso o recurso ficaria inerte dentro de um `tmux`/`vim` com mouse tracking ligado. `[terminal.hyperlinks]` nasceu em `porecatu-config` (`Hyperlinks { enabled: bool }`, default `true`) e a seção do arquivo de exemplo foi descomentada na mesma leva — a única chave, sem lista de esquema configurável (ADR-0042 §5). Testes: `porecatu-ui` 270→289 (19 novos: 13 em `hyperlink.rs`, 6 em `terminal_menu.rs`); `porecatu-config` ganhou `terminal::hyperlinks` com o teste de `default_matches_example_toml` de sempre, e `example_toml.rs`/`verify-docs.py` continuam batendo.

   **Nenhum desvio do ADR-0042.** A política de esquemas não foi relaxada em ponto nenhum — allowlist de quatro, `file` revelado nunca aberto, lista não configurável. O único ponto onde o código foi além do texto literal do ADR foi corrigir a premissa errada sobre `UNDERLINE` já descrita acima, que é correção de fato, não mudança de decisão (regra do processo de ADR em `CLAUDE.md`).

   **Verificação por gesto, e o que ela ensinou sobre a proteção de foco.** Mouse sintético atravessa a proteção de foco do Windows para **clique** (confirmado com evidência: `--config`/diretório descartável isolando a instância de teste da sessão real, PID conferido antes de qualquer captura, região da janela recortada pelo `GetWindowRect` daquele PID) — o clique secundário sobre um `file://`/uri OSC 8 emitido por um script de shell no spawn abriu o menu de contexto do terminal de verdade, com os itens certos ("Colar" já realçado como primeiro habilitado, sem seleção). O que a sessão **não** conseguiu confirmar por gesto foi o `Ctrl`+clique: o clique em si chega à janela sem foco (confirmado), mas o estado do modificador `Ctrl` que o `winit` lê depende de `WM_KEYDOWN`, que só chega à janela com foco de teclado — e um clique sintético (`mouse_event`) não transfere foco de teclado para uma janela em segundo plano da mesma forma que um clique real do usuário transfere (`GetForegroundWindow` continuou apontando pro processo que envia o clique, não pro Porecatu, mesmo depois do clique). Achado novo, não estava registrado antes: **clique sintético atravessa a proteção de foco, `Ctrl` sintético mantido durante o clique não**. Fica como dívida de verificação a confirmação do `Ctrl`+clique por gesto real do usuário ou por uma sessão com uma ferramenta de automação que consiga transferir foco de teclado de verdade.

   **Bug achado, não fechado: texto de item desabilitado do menu quase invisível.** "Copiar" sem seleção usa `[appearance.context_menu] disabled_foreground` (`#5c646f`) exatamente como os outros dois menus seriam capazes de fazer — só que nenhum item deles nasce desabilitado (`TAB_MENU_ITEMS`/`GROUP_ACTION_ORDER` são sempre `enabled: true`), então este é o **primeiro uso real** dessa cor em texto na história do app. Confirmado por instrumentação (`eprintln!` temporário, removido antes do commit) que a `Color` que chega em `Primitive::Text` é `(0.36, 0.39, 0.44, 1.0)` — exatamente `#5c646f`, alpha cheio — mas o pixel final na tela sai quase idêntico ao fundo do menu (`#1a1e25`), bem abaixo do contraste que a mesma cor produz nos outros widgets. A cor certa chega a `porecatu-render`; o que sai errado é depois disso, no pipeline de `glyphon`/`cosmic-text` (`text.rs`), fora do que esta etapa tem escopo para investigar a fundo. Item continua presente e navegável (RF-11.15: "esmaecido, não ausente" -- ausente ele não está, e pular ele por teclado/mouse quando desabilitado já funciona), só ilegível. Registrado para investigação em `porecatu-render`, não em `porecatu-ui`.
4. **Os requisitos aprovados e não entregues — fechada para quatro dos seis itens, dois cortados com o alcance investigado.** Seis assuntos independentes, um commit cada, nenhum novo mecanismo por trás: os quatro entregues reusam exatamente os caminhos que fases anteriores já abriram.

   **RF-11.24 — avisos de config no arranque.** `App::new` ganhou `pending_startup_warnings: Vec<(Severity, &'static str, String)>`, montado no lugar dos três `eprintln!` (config inválida, chave desconhecida, tema desconhecido) — mesmo texto e severidade que `apply_config_reload` já usa no hot reload, só a origem muda. Entregue pelo mesmo caminho que a etapa 4 da F5 abriu para `Notice::Corrupt`/`NewerSchema`: consumido em `resumed`, na primeira janela criada. **Bug achado e corrigido no próprio caminho**: a verificação por gesto (ver abaixo) mostrou a pilha vazia ao rodar com um diretório posicional (RF-3.12) — `resumed` retorna cedo nesse modo, antes do bloco que entregava os avisos, porque a leitura de sessão de fato não roda ali; mas a leitura de **config** roda sempre, em `App::new`, independente do modo. `deliver_pending_startup_warnings` foi extraído para um método próprio e passou a ser chamado nos dois caminhos de `resumed` (posicional e normal) — sem ele, RF-11.24 nunca aparecia para quem sobe o app com um diretório na linha de comando. Sem teste unitário novo (mesma fronteira de `App`/`WindowState` sem GPU/janela que toda fase anterior já registra); verificado por gesto real.

   **RF-11.25 — família de fonte ausente.** `TextMeasurer::build` (`porecatu-render`) passou a checar, antes de mover o `fontdb::Database` para dentro do `FontSystem`, se `families.mono` bate com alguma face carregada (embutida ou do sistema) — `db.faces().any(...)`, sem depender de `Query`/fallback (que resolveriam via cadeia comum e mascarariam a ausência). `missing_mono_family()` devolve o nome exato configurado quando não bate; `porecatu-ui` lê isso uma vez, no primeiro `GpuContext::new` do processo, e empilha o aviso. O `#[allow(dead_code)]` em `Severity::Warning` (`warning.rs`), estagnado desde a F4 com a justificativa "são F4", saiu — a variante já era usada de fato desde o hot reload (chave desconhecida, tema desconhecido), então o `allow` já estava morto antes mesmo deste item; removido na mesma leva. Testes: `porecatu-render` 33→35 (`default_family_is_never_missing`, `unknown_family_is_reported_by_exact_name`).

   **RF-11.26 — fallback de GPU sem `panic`.** `GpuContext::new_async` pedia adapter uma vez e dava `.expect()`; agora, se a primeira tentativa falha, uma segunda pede explicitamente `force_fallback_adapter: true` antes de desistir — só essa segunda falhando é `panic` (sem GPU e sem software nenhum, cenário sem recuperação). `software_rendering()` é `true` nesse caminho **ou** quando a primeira tentativa já devolve direto um adapter `DeviceType::Cpu` (algumas VMs expõem só isso sem a primeira falhar). Avisado uma vez, no mesmo ponto do item anterior. **Não verificado por gesto**: as duas tentativas de forçar o caminho por variável de ambiente (`WGPU_ADAPTER_NAME`) não se aplicam — esse env var só é lido por `wgpu::util::initialize_adapter_from_env*`, que `gpu.rs` não chama (o código sempre chamou `Instance::request_adapter` direto); forçar de verdade pede VM sem aceleração ou um `Backends` restrito sem adapter compatível, nenhum dos dois disponível nesta sessão. Fica como dívida de verificação, registrada como tal, não como "passou" — a mesma classe de dívida que o ADR-0038 já assume para o fallback de `cwd` fora do Windows.

   **RF-11.27 — botão de configurações deixa de ser inerte.** `NewTabRequest` ganhou `OpenConfigFile`: `handle_bar_click` já resolvia o hit-test do botão (zona fixa à direita) e só trocou o `NewTabRequest::None` que consumia o clique por esse pedido, que sobe para `App` (só ele tem `config_path`). `open_config_file` reusa o **mesmo** crate `opener` que a etapa 3 adotou para hyperlink — `opener::open(path)`, caminho como argumento, nenhuma string de shell, nenhum `unsafe` novo. Arquivo ainda não existente: criado a partir do exemplo embutido (`include_str!` de `docs/config/porecatu.example.toml`, mesmo padrão de `shell_integration::REFERENCE_MD`) antes de abrir — decisão tomada como o próprio item previa ("a opção óbvia"), sem precisar perguntar. `ensure_config_file_exists` ficou separada de `open_config_file` de propósito: testável sem `opener` (testar a chamada real abriria um programa de verdade na máquina que roda `cargo test`). Testes: `porecatu-ui` 289→292 (`missing_file_is_created_from_the_embedded_example`, `existing_file_is_left_untouched`, `missing_parent_directory_is_created`).

   **Cortados, com o alcance investigado — decisão do usuário, registrada aqui.** RF-11.28 (`zoom_scope = "active"`): `cell_metrics` é hoje um valor único por processo (`App::cell_metrics`, ADR-0015: "uma medição serve todas as janelas"); zoom por aba de verdade — cada `TabRuntime` com seu próprio tamanho, recalculado/re-resize a cada troca de aba — empurraria isso de "por processo" para "por janela" (mover o campo de `App` para `WindowState`, mexendo em resize de PTY, hit-test de mouse e pintura, uns 15-20 pontos de chamada), e ainda esbarraria em `WindowV1::zoom_steps`, hoje por janela: salvar zoom por aba mudaria o schema de sessão (ADR-0036). Investigado, não implementado — cortado antes de tocar código, por decisão do usuário diante do alcance. RF-11.29 (cor hexadecimal no editor de grupo): `porecatu_core::GroupColor` é enum fechado de seis variantes, `#[derive(Serialize, Deserialize)]` direto, gravado hoje em `session.json` via `GroupV1.color: Option<String>` (`convert::color_to_str`/`color_from_str`, seis nomes fixos). Aceitar hexadecimal pede um sétimo caso no tipo do **core**, e esse tipo é exatamente o que atravessa o schema de sessão versionado — mudança que o ADR-0036 rege, não trabalho de etapa. Investigado, não implementado — cortado pelo mesmo motivo. Os dois seguem exatamente como a dívida da etapa 6 da F4 já os registrava (ver acima); nenhuma linha do ADR-0032 §50 muda, porque nada sobre cor hexadecimal foi decidido aqui.

   **Verificação por gesto.** `cargo build/test/clippy/fmt --workspace` e `python scripts/verify-docs.py` verdes. Instância de teste isolada por `--config`/diretório posicional descartável e `PORECATU_SESSION` de rascunho (nunca a sessão real), PID conferido antes de qualquer captura, região recortada pelo `GetWindowRect` daquele PID — mesma disciplina das etapas anteriores. Confirmado ao vivo: (1) config sintaticamente inválida produz exatamente um aviso com barra de severidade **vermelha** (`Severity::Error`); (2) chave desconhecida + tema desconhecido + família de fonte ausente, todos na mesma config, produzem exatamente três avisos empilhados com barra **laranja** (`Severity::Warning`), na ordem esperada; (3) o botão de configurações, clicado de verdade (mouse sintético, que atravessa a proteção de foco do Windows), criou o arquivo a partir do exemplo embutido e abriu de fato no editor padrão do sistema (VS Code, neste ambiente) com o conteúdo certo na tela — janela nova, foco real transferido para ela (confirmado por `GetForegroundWindow`), fechada depois pelo mesmo processo que a abriu. RF-11.26 não verificado por gesto (ver acima).

   **Bug achado, não fechado — mais grave e mais amplo do que a etapa 3 tinha registrado.** A etapa 3 documentou texto de item desabilitado de menu "quase invisível" e não investigou além de `porecatu-render`. Esta etapa exercitou pela **primeira vez** — nenhuma fase anterior verificou por captura de tela a pilha de avisos do ADR-0014, sempre deixada como dívida de verificação — o widget inteiro com conteúdo real, e o mesmo problema apareceu ali: instrumentação temporária (`eprintln!`, removida antes do commit) confirmou que título (`#dfe4ea`, quase branco) e corpo (`#6b737e`) chegam corretos em `Primitive::Text`, na posição certa, com o texto certo — e mesmo assim os três avisos aparecem como cartões vazios na tela, só a barra de severidade colorida visível. Ou seja: **não é** um problema de contraste específico da cor de item desabilitado (a hipótese da etapa 3) — é algo mais geral no pipeline de texto de `porecatu-render` (`text.rs`, `glyphon`/`cosmic-text`) que afeta texto proporcional em pelo menos dois widgets distintos com cores bem diferentes. Continua fora do escopo desta etapa investigar a fundo; registrado como correção do escopo do achado da etapa 3, não como achado novo separado.
5. **Acessibilidade — fechada.** `accesskit` 0.25/`accesskit_winit` 0.34 (a versão dita pelo `winit 0.30.13` já pinado, não o contrário — mesma disciplina do `wgpu`), `winit` continua aparecendo **uma** vez em `cargo tree -p porecatu-ui -i winit` (confirmado). `porecatu-ui` ganhou `access.rs`: a árvore é montada só a partir das mesmas funções puras de layout que produzem o desenho (`tab_bar::layout`/`overflow_state`) — nenhuma segunda fonte de verdade, `porecatu-render` não participa. `WindowState` ganhou `access_adapter: accesskit_winit::Adapter` (um por janela, criado com a janela ainda invisível — `base_window_attributes` ganhou `with_visible(false)`, `create_window_with_attributes` só chama `set_visible(true)` depois do adaptador existir, como o construtor exige). `Wakeup` ganhou a variante `AccessKit(accesskit_winit::Event)` (perdeu `Clone`/`PartialEq`, que o tipo do evento não implementa e nada usava); `window_event` chama `process_event` antes de qualquer despacho nosso; a árvore inicial é montada em resposta a `WindowEvent::InitialTreeRequested`, e toda atualização seguinte acontece dentro de `schedule_next_wake` — o mesmo ponto único que já drena sessão/redraw, nunca dentro do caminho de render, nunca pedindo frame. `update_if_active` decide sozinho se há cliente conectado: sem leitor de tela, a função de montagem da árvore nem roda.

   O que a árvore expõe (ADR-0043 §4): barra de abas completa — título, posição na ordem visual, grupo, estado (ativa/atividade/campainha/não iniciada), pílula com nome/cor/colapso, botão de fechar, botão de nova aba por grupo, indicador de overflow, botão de configurações, botões de janela; os cinco widgets — aviso (`Role::Alert`, severidade no texto), diálogo (`Role::Dialog` + `set_modal`, foco no botão certo via `TreeUpdate::focus`), os três menus de contexto concretos (aba/grupo/terminal, `Role::Menu`+`Role::MenuItem`, item desabilitado presente e marcado, nunca ausente) mais o popover de mover-para-grupo (mesma forma), e o editor de grupo (`Role::Group` com campo de nome, faixa de swatches como `Role::RadioGroup` e lista de ações); e a barra de busca (`Role::SearchInput` com o contador como descrição, alternador de regex como `Role::Switch`). A grade do terminal fica fora — nenhuma função do módulo recebe `GridSnapshot`. Sem travessia por `Tab` somada (ADR-0043 §6): nenhuma tecla nova capturada no nível do app.

   Simplificações registradas (dívida nomeada, não ADR novo): sem `bounds` por nó — nenhum nó carrega retângulo de tela, então um leitor que dependa de `BoundingRectangle` (exploração por toque, ímã de lupa) não tem onde ancorar; navegação por cursor virtual/teclado, o caminho que este PR verificou, não depende disso. Sem `TextSelection` de caractere nos campos de texto (nome de grupo, busca) — só `Value`. Sem despacho de `ActionRequest` de volta ao app — a árvore expõe, não aciona; RF-11.17/18 pedem o primeiro. Testes: `porecatu-ui` 292→299 (7 novos em `access.rs`, função pura, incluindo o que o ADR pede: `building_the_tree_never_touches_window_or_gpu`, que prova por assinatura de tipo — nenhum parâmetro é janela ou GPU — que a função não tem como pedir frame).

   **Verificação por gesto, real e ao vivo.** NVDA não estava instalado nesta máquina; a verificação rodou contra a mesma API que ele consome no Windows — `System.Windows.Automation` (UI Automation), via instância isolada (`--config`/diretório descartável, `PORECATU_SESSION` de rascunho, PID conferido antes de cada consulta). Confirmado de verdade, sem simular nada do lado do app: a janela aparece como `ControlType.Window` "Porecatu"; a aba única aparece como `TabItem` com o título real do processo e "(ativa)"; criar uma segunda aba por clique sintético (que atravessa a proteção de foco do Windows, achado da F4) fez as duas aparecerem, a certa marcada ativa; o menu de contexto da aba (clique direito sintético) apareceu como `Menu` com os três `MenuItem` exatos, na ordem certa; uma config sintaticamente inválida produziu um nó de aviso com o texto `"Erro: Config inválida: <mensagem real do parser>"`; e — o caso que o ADR chama de mais sério, "não perder um diálogo modal que apareceu" — fechar a janela com duas abas abriu o diálogo como `ControlType.Window` "Fechar janela?" com os dois botões, e **`AutomationElement.FocusedElement`, a própria API do Windows que um leitor de tela consulta para saber o que anunciar, devolveu "Cancelar"** — confirmação de terceiro, não do nosso próprio código, de que o foco inicial (ADR-0014) chega corretamente à árvore de acessibilidade.

   **Dívida de verificação, registrada.** NVDA em si (o anúncio falado) não foi ouvido — instalar um leitor de tela de verdade nesta máquina não foi feito sem pedir, e a verificação por UI Automation acima cobre o mesmo contrato que ele consultaria. Não exercitados por gesto real (mesma forma estrutural dos que foram, risco baixo): editor de grupo, menu de grupo, popover de mover-para-grupo, barra de busca. macOS e Linux entram só por compilação — `accesskit_winit` resolve para as três plataformas (conferido na abertura da fase), mas este fluxo só tem Windows.

   **Nenhum desvio do ADR-0043.**
6. **Métricas, empacotamento e release — fechada, e com ela a F6 e o v1 inteiro.** Quatro partes independentes, mais o fechamento da fase.

   **Parte A — instrumentação e medição.** `porecatu-ui` ganhou `trace.rs`: três pontos de `Instant` atrás de `PORECATU_TRACE` (variável de ambiente, não flag — a superfície de `argv` do [ADR-0040](adr/0040-superficie-de-linha-de-comando.md) continua pequena de propósito), custo zero sem a variável (`trace::enabled()` é `OnceLock`, conferido antes de qualquer `Instant::now()`). Os cinco números do [PRD-000](prd/prd-000-visao-de-produto.md), medidos numa máquina de desenvolvimento comum (não um ambiente controlado de benchmark — números de ordem de grandeza, não de laboratório):

   | Métrica | Alvo | Medido | Bateu? |
   |---|---|---|---|
   | Tempo até o primeiro prompt utilizável | < 300 ms | **~500 ms** (pwsh como shell default) / **~375 ms** (`cmd.exe` explícito) | **Não** |
   | Uso de CPU com todas as abas ociosas | ~0% | **0.52%** (amostra de 3s, processo real, sem input) | Sim (contra "~0%", não zero literal) |
   | Restauração de sessão com 20 abas até janela interativa | < 1 s | **~502 ms** | Sim |
   | Latência de tecla até pixel | < 16 ms | **~16.0 ms** (15.97 ms, uma amostra) | Sim, no limite |
   | Reconstrução manual de contexto após reabrir | zero ações do usuário | zero (restauração roda sozinha em `resumed`, sem diálogo nem confirmação) | Sim, por construção |

   A primeira **não bateu o alvo**, e o número revela por quê: `main -> primeiro byte do PTY` é ~480ms com o shell default desta máquina (`pwsh` via MSIX/Store — o mesmo pwsh que o CLAUDE.md já registra como o de instalação atípica) e ainda ~358ms trocando para `cmd.exe` explícito no `[shell] program` — o shell mais leve que o Windows tem. `primeiro byte do PTY -> primeiro frame` é consistentemente ~17-19ms nas duas medições, ou seja, **o app não é o gargalo**: a fatia dominante é o tempo entre `main` e o primeiro byte do PTY, que cobre `App::new` (config, keymap, `EventLoop::build`), a criação da janela e do `GpuContext` (primeiro `wgpu` adapter + pipelines) e o `spawn` do processo do shell via ConPTY — sem uma instrumentação mais fina *dentro* desse trecho (fora do escopo desta etapa, que instrumentou só os três pontos que o ADR já nomeava), não dá para separar quanto é inicialização de GPU e quanto é o próprio ConPTY/shell. **Não foi corrigido nesta etapa** — é decisão do dono do produto se vale abrir uma etapa nova para reduzir isso, e com quê prioridade, à luz do resto da dívida do v1. A quarta métrica bateu **por uma margem de nada** (15.97ms contra um alvo de 16ms, uma única amostra) — não dá para chamar de folgada.

   A restauração de sessão fecha a dívida que a etapa 4 da F5 tinha deixado sem medir: com `lazy_restore` (padrão), só a aba ativa sobe de verdade — as outras 19 nascem `NotStarted`, sem PTY nenhum —, e é exatamente esse mecanismo que faz a métrica caber tão longe do 1s mesmo com o mesmo custo de spawn de shell da primeira métrica embutido nela.

   **Parte B — empacotamento** ([ADR-0044](adr/0044-empacotamento-e-release.md)). Instalador nativo por plataforma, nenhum recompila o binário (critério 2 do ADR-0044 §2 — todos consomem o executável que o passo de compilação já produziu): MSI no Windows (`cargo-wix` 0.3.9, `--no-build`, `wix/main.wxs` versionado); `.deb` (`cargo-deb` 3.8.0, `--no-build`) e AppImage (montado à mão, `appimagetool` 1.9.1 — a tag real, não a `continuous` móvel, conferida por `sha256`) no Linux; `.app` dentro de `.dmg` no macOS, montado por script próprio (`sips`/`iconutil`/`hdiutil`, já no runner) — **sem** `cargo-bundle`, que recompilaria com flags próprias na versão corrente (sem `--no-build`) e violaria o critério 2. `x86_64-apple-darwin` entra na matriz, cross-compilado no mesmo runner `arm64` do `macos-latest` (a correção que o ADR-0044 trazia junto: Mac Intel ficava de fora sem decisão registrada em lugar nenhum). `--locked` passa a valer também no `ci.yml` (job `rust` e job `canario`) — última pendência aberta desde a F0. Cada plataforma tem um passo que reprova o job se a atribuição das fontes não estiver no artefato publicado (RF-11.21): `dpkg-deb -c` no `.deb`, teste de arquivo no `AppDir`/`staging` antes de empacotar, e extração administrativa do MSI (`msiexec /a`) verificada por `Get-ChildItem -Recurse`.

   **A parte Windows foi verificada de ponta a ponta, ao vivo, nesta máquina** — o único ramo do empacotamento que dava para testar sem CI real: WiX Toolset v3.14.1 baixado à parte (a mesma versão que o `windows-latest` já traz pré-instalada, sem passo manual), `cargo wix --no-build` gerou o MSI de verdade contra um binário `--release` desta etapa, e `msiexec /a` (instalação administrativa) confirmou que o executável, `LICENSE`, as duas atribuições de fonte e `porecatu.example.toml` estão todos dentro do MSI publicado — exatamente o que RF-11.21 exige. `.deb`, AppImage e `.app`/`.dmg` **não foram exercitados** (sem ambiente Linux/macOS local, e disparar o `release.yml` de verdade exigiria a mesma autorização que publicar uma release) — só sintaxe de shell e o mapeamento dos flags de cada ferramenta contra a documentação oficial delas, sem rodar de fato.

   **Parte C — versão.** `version.workspace` sobe de `0.1.0` para **`0.7.0`** — não `1.0.0`, como o ADR-0044 §3 tinha decidido: o [ADR-0045](adr/0045-primeira-versao-0-7-0.md) revisa esse número (supersede parcial, só §3) por decisão do dono do produto ao fechar a fase, com a dívida de verificação acima como parte do motivo.

   **Parte D — documentação de usuário** (RF-11.22). [`docs/guia-do-usuario.md`](guia-do-usuario.md): instalação por plataforma com a tabela de artefatos, como conferir o `sha256`, o que esperar de SmartScreen e Gatekeeper (ADR-0044 §5, sem assinatura no v1) e por que isso não substitui a verificação de integridade; caminho do arquivo de config por plataforma e a precedência `--config`/`PORECATU_CONFIG`; atalhos mais usados, linkando o catálogo fechado; a convenção do `Shift` para forçar seleção local dentro de `vim`/`htop`/`fzf` — exigência nominal do [ADR-0013](adr/0013-mouse-selecao-e-clipboard.md); e integração de shell, linkando [docs/reference/integracao-de-shell.md](reference/integracao-de-shell.md) em vez de duplicar os snippets. RF-11.23 (notas de release) não pediu código: `generate_release_notes` do `release.yml` continua como complemento, e o `CHANGELOG` (fechado nesta mesma leva) é a fonte.

   **Nenhum desvio do ADR-0044** além do que o ADR-0045 já revisa e registra explicitamente (o número da versão). Testes: nenhum novo (a etapa é instrumentação e infraestrutura de build, não lógica de domínio nova) — `cargo test --workspace` continua em 603 (soma de todos os crates), inalterado desde a etapa 5.
...
**Critério de saída:** todos os cenários de aceite de PRD-011 passam; **leitor de tela navega a barra de abas e os cinco widgets** — emendado de "a barra de abas", pelo [ADR-0043](adr/0043-arvore-de-acessibilidade.md), que fechou o escopo em cinco papéis em vez de um; as cinco métricas de [PRD-000](prd/prd-000-visao-de-produto.md) **medidas por instrumentação** — quatro atingidas, uma não (tempo até o primeiro prompt utilizável; ver a etapa 6 para o número e o que ele revela); busca num scrollback cheio sem travar a UI; nenhum esquema de URI fora dos quatro é executável por um gesto; instaladores nativos publicados para as três plataformas, com a atribuição das fontes dentro (o Windows verificado de ponta a ponta nesta máquina; `.deb`, AppImage e `.app`/`.dmg` não exercitados, ver dívida abaixo).
...
**Dívida herdada de verificação, com o que realmente aconteceu.** A acessibilidade foi verificada ao vivo (etapa 5), mas **não** com o NVDA que a previsão acima esperava — não estava instalado nesta máquina, e instalá-lo sem perguntar seria mudança de sistema maior do que o pedido autorizava. A verificação rodou contra `System.Windows.Automation` (UI Automation) direto, a mesma API que o NVDA consome no Windows: janela, aba, menu de contexto, aviso e diálogo confirmados ao vivo, incluindo `AutomationElement.FocusedElement` confirmando o foco inicial do ADR-0014. Editor de grupo, menu de grupo, popover e busca não foram exercitados ao vivo (mesmo caminho estrutural do que passou, risco menor). macOS e Linux entraram só por compilação, como previsto. **Instalação real dos instaladores**: o MSI do Windows foi o único ramo verificável sem CI de verdade, e foi — WiX Toolset baixado à parte, `cargo wix` gerou o MSI contra um binário `--release` desta etapa, e `msiexec /a` confirmou o conteúdo publicado. `.deb`, AppImage e `.app`/`.dmg` continuam só como sintaxe conferida contra a documentação das ferramentas, nunca rodados de verdade — abrir o `release.yml` via `workflow_dispatch` numa tag de rascunho, como a etapa pede, é ação que publica de verdade e exige o mesmo OK explícito da tag. Mouse sintético continua atravessando a proteção de foco do Windows (descoberta da F4); teclado sintético, que a F1 sempre registrou como bloqueado, **funcionou** para a instrumentação desta etapa (`SetForegroundWindow` teve sucesso e uma tecla real chegou à janela, medindo "tecla -> pixel") — não foi usado para nenhum gesto de teclado do produto (atalho, digitação na busca), só para essa medição pontual.
...
**Depois, o segmento de Git** ([ADR-0049](adr/0049-branch-git-na-barra-de-status.md), RF-9.9): ícone de repositório mais o nome da branch, lidos de `.git/HEAD` e revalidados por `mtime` -- sem thread, sem temporizador e sem dependência nova. O estado da árvore (sujo/limpo, ahead/behind) **continua fora**, e é fronteira, não pendência: ele precisa percorrer a árvore, e nada barato diz quando refazer.
...
## Depois do v1 — comando de projeto por diretório — implementada

Fora da ordem de fases, como a barra de status, mas por um caminho diferente: aqui não houve rascunho a promover. O [PRD-012](prd/prd-012-comando-de-projeto-por-diretorio.md) é **requisito novo**, escrito depois de o v1 estar em uso e a pedido do dono do produto, e o [ADR-0051](adr/0051-arquivo-de-projeto-porecatu.md) fecha as cinco decisões que ele deixa em aberto.

A razão de existir é uma métrica que o v1 declarou atingida e não estava. O [PRD-000](prd/prd-000-visao-de-produto.md) define o produto por "reconstrução manual de contexto após reabrir: **zero ações do usuário**", e a instrumentação da etapa 6 da F6 mediu isso contra a **estrutura** restaurada — abas, grupos, diretórios. Contra o **trabalho** retomado, o número é outro: quem reabre com quinze abas ainda digita quinze comandos, porque a sessão devolve onde a aba estava e não o que ela fazia. O `.porecatu` é onde o projeto declara isso, versionado junto com ele, com uma seção por shell.

**Quatro etapas, todas fechadas:**

1. **Documentação e decisão** — PRD-012, ADR-0051, a referência do formato em [docs/reference/arquivo-de-projeto.md](reference/arquivo-de-projeto.md), a emenda no PRD-003 (que proíbe replay e continua valendo), `[project_file]` no arquivo de exemplo, o guia do usuário e os índices. Sem novidade em relação ao planejado.
2. **Formato e confiança** — `project_file.rs` em `porecatu-config`: parser de seções cruas (`section_header`/`push_section`/`parse`, sem estado de erro), escolha da seção por `shell_name` com `[default]` de último recurso (`ProjectScript::command_for`), e `is_trusted` com canonicalização dos dois lados e comparação por componente de `Path::components()`, nunca por `starts_with` de string. `ProjectFileOutcome` (`Disabled`/`NotFound`/`Untrusted`/`Unreadable`/`NoSection`/`Ready`) é o contrato que a etapa 3 consome. `[project_file]` em `Config`, classe de recarga [C]. 21 testes: parser (preâmbulo, seção única/multi-linha, linha em branco no meio preservada e no fim cortada, `#` literal, `[algo]` no corpo virando seção nova, casamento sem diferenciar maiúsculas, `[default]` de último recurso, `\r\n` normalizado, seção duplicada com a primeira vencendo) e confiança (lista vazia nega tudo, caminho exato, subdiretório, `..` escapando a árvore negado, `C:/Projetos` × `C:/Projetos-do-vizinho` não colidindo, entrada inexistente ignorada em silêncio, `~` expandido, `enabled = false` sem tocar disco, não autorizado vencendo sobre conteúdo ilegível). Sem desvio do planejado.
3. **Gatilho e execução** — `SpawnOrigin` (`New`/`Restored`) como parâmetro explícito de `spawn_tab_runtime`, nunca inferido, forçando todo `open_tab`/`ensure_active_tab_started`/`open_window_from_session` a decidir; `shell_name` usado para casar a seção é sempre o do shell que **vai** subir (`Self::shell_display_name(&config.shell)`), nunca o `Tab::shell_name` gravado na sessão — os dois divergem se o usuário trocou de shell entre execuções. `PendingProjectCommand` e o critério puro `project_command_timing` (primeiro byte + ~300ms de silêncio, teto de 5s, guarda de tela alternativa que o teto sempre vence) alimentados por `Instant` de fora, nunca por `Instant::now()` do próprio módulo — mesma disciplina do resto do projeto. `pending_project_notice` para o arquivo não autorizado e uma flag `project_file_notice_claimed` de app inteiro (não por aba). Escrita linha a linha terminada em `\r`, igual ao Enter. Todas as funções de decisão extraídas puras e testadas sem `Terminal`/PTY real — inclusive os dois cenários mais delicados: `SpawnOrigin::New` nunca gera pendência mesmo com `.porecatu` autorizado presente, e duas abas com nota pendente produzem uma só.
4. **Verificação e guia** — ver "Dívida de verificação" abaixo para o que rodou de ponta a ponta. Achou e corrigiu um bug real (ver a seção de descobertas do [CLAUDE.md](../CLAUDE.md)): a nota de diretório não autorizado só existia como `Option<PathBuf>` sem contribuir nenhum `Instant` para `schedule_next_wake`, então só aparecia por coincidência com outro temporizador do processo (o debounce de gravação de sessão, ~2s por default) — às vezes segundos depois do foco na aba, quando devia ser imediata. Corrigido movendo a checagem de `tick_all` (só roda em `ResumeTimeReached`) para `schedule_next_wake` (roda a cada volta do event loop, sem depender de nenhum prazo agendado) — a nota não tem critério de tempo, ao contrário do comando pendente, então não precisa esperar por um.

**Escopo:** RF-12.1 a RF-12.15, todos. Nada diferido — o PRD foi escrito já sem o que ficaria de fora.

**Aparência:** nenhuma. Nenhum widget novo, nenhum valor novo, nenhuma cor nova; a nota do não autorizado usa o canal 2 do [ADR-0014](adr/0014-superficie-de-aviso-e-dialogo.md) e a marcação do [ADR-0039](adr/0039-convite-a-integracao-de-shell.md). O [ADR-0032](adr/0032-interface-do-v1-fechada.md) continua fechado e a especificação visual não foi tocada. Nenhuma dependência nova, nenhum `unsafe`, e a tabela de stack do README não muda.

**Critério de saída: atendido e demonstrado ao vivo** — ver "Dívida de verificação" para o que ficou só em teste automatizado. Uma aba restaurada num diretório autorizado executa o script da seção do seu shell, visível no prompt e no histórico; a mesma aba num diretório **não** autorizado não executa nada e informa uma vez por execução do app; aba nova no mesmo diretório e `cd` para ele não disparam nada; `trusted_paths = []` (o default) não executa nada em lugar nenhum; um caminho com `..` ou um vizinho de prefixo textual não passa pela comparação; e `verify-docs.py` (conferido à mão nesta máquina, sem Python instalado) e o CI verdes nas três plataformas.

**A decisão que mais pesou**, e que está no [ADR-0051](adr/0051-arquivo-de-projeto-porecatu.md) §4 com a alternativa recusada: a confiança é uma **allowlist declarada na config do usuário**, vazia por default. A alternativa levada ao dono do produto era autorizar por diretório dentro do terminal, no modelo do convite do ADR-0039 — mais granular, e reagindo à mudança do arquivo depois de autorizado. Foi recusada. O custo da escolhida está registrado como consequência negativa e como primeira linha da tabela de riscos: não há caminho de UI para autorizar, porque o app não escreve na config, e um `trusted_paths` largo é uma decisão que se toma uma vez e se esquece.

**Dívida de verificação.** A suíte automatizada cobre parser, confiança (inclusive `..`, symlink e o par de prefixo textual) e as cinco funções puras de temporização/decisão da etapa 3 — 706 testes no workspace, todos verdes, `clippy -D warnings` limpo. Ao vivo, nesta máquina (Windows, sem `pwsh.exe` no `PATH` — o shell resolvido foi `powershell.exe`), com sessão gravada à mão (`session.json` construído com o schema v1 e carregado por `PORECATU_SESSION`, em vez de fechar e reabrir o app de verdade) e teclado/mouse sintéticos confirmados por foco real a cada chamada:
- **Confirmado**: o caso motivador (diretório autorizado, `npm`-like script escrito e executado no prompt de uma aba restaurada); diretório não autorizado (nenhuma execução, uma nota só, com o caminho e a linha de config certos); aba nova (`Ctrl+Shift+T`) no mesmo diretório autorizado não executa nada; restauração preguiçosa com três abas (só a ativa roda no arranque, as outras duas rodam — ou mostram a nota — no primeiro foco, uma por `Ctrl+Tab`); o comando aparece no histórico do shell (seta para cima recupera) e `Ctrl+C` o interrompe antes do fim.
- **Não exercitado**: `bash`/`zsh`/`fish`/`sh` (só `pwsh`/`powershell`/`cmd` têm seção prevista no arquivo de exemplo e nenhum dos três primeiros está disponível nesta máquina Windows); Linux e macOS, mesma dívida herdada de todas as fases anteriores; um `.porecatu` genuinamente ilegível (`Unreadable`) não foi provocado ao vivo (só em teste automatizado), embora o caminho de código seja o mesmo do `missing_cwd_note` já verificado noutras fases.
