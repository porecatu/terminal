...
**Estado atual: F0, F1, F2, F3 e F4 fechadas.** A verificação interativa das três fases é **dívida assumida**, não critério pendente: a proteção de foco do Windows bloqueia input sintético e não há ambiente Linux/macOS no fluxo, então elas fecharam com cobertura automatizada mais smoke test (ver a nota no topo do roadmap). `cargo run` abre uma janela com abas de terminal funcionais e **grupos**: PTY, motor VT, render por GPU, teclado, mouse, seleção, clipboard, ciclo de vida de aba, overflow da barra, arraste, seleção múltipla, pílula e cápsula de cor, colapso, editor de grupo, menu de contexto de grupo, popover de destino, arraste entre grupos e do grupo inteiro, animação de reflui, tooltip, aviso e diálogo de confirmação — e `Ctrl+Shift+N` abre uma segunda janela. Fora do macOS, a janela também perdeu a decoração nativa: drag region e botões de janela na própria barra de abas, resize por toda borda ([ADR-0027](docs/adr/0027-controles-de-janela-e-resize-proprios.md), pedido do usuário, fora de fase). **A F4 fechou em seis etapas**, na branch `feat/porecatu-ui-le-config`: `porecatu-config` nasce (etapa 1); `porecatu-ui` lê `Config` — chrome e barra de abas saem de `const` (etapa 2); terminal — fonte, cores, cursor, scrollback, seleção, clipboard, `[general] startup_directory`, `[shell]` (etapa 3); hot reload — `notify` assistindo o diretório do arquivo, debounce de ~200ms, as três classes do [ADR-0030](docs/adr/0030-escopo-do-hot-reload.md) por diff entre config antiga e nova (etapa 4); `enum Action` em `porecatu-core` e resolução de `[keybindings]` em três níveis via [ADR-0029](docs/adr/0029-enum-de-acao-e-gramatica-de-tecla.md) (etapa 5); temas nomeados, zoom de sessão, `animations = false`, roda do mouse no popover de destino, e as duas mudanças visuais aprovadas — hover por brilho e sombra nos cinco widgets de chrome ([ADR-0031](docs/adr/0031-temas-nomeados.md), [ADR-0032](docs/adr/0032-interface-do-v1-fechada.md), etapa 6). **Dívida registrada da etapa 6**, sem ADR pendente para resolver, só trabalho: o merge de tema (`porecatu_config::apply_theme`) só cobre os campos que `Theme` já tinha desde a etapa 1 (16 ANSI + 10 nomeados) — `[appearance.groups]` (`palette`/`ungrouped_color` inclusos) e as cores dos cinco widgets de chrome, que o ADR-0031 §1 também inclui no escopo de tema, ainda não são mesclados; o zoom de sessão (`font.increase`/`decrease`/`reset`) é sempre do processo inteiro, `zoom_scope = "active"` não tem efeito ainda (zoom por aba pediria métrica de célula por `TabRuntime`, não só por processo); a entrada de cor por hexadecimal no editor de grupo (RF-2.10) não foi implementada; e a flag `--config` (dívida da etapa 1) tem a precedência pronta em `resolve_config_path`, mas nada em `main.rs`/`App::new` lê `argv` pra chamá-la com um caminho — `App::new` chama `porecatu_config::load(None)` fixo, então só `PORECATU_CONFIG` e o caminho padrão da plataforma respondem hoje. `porecatu-session` continua stub. Valor de aparência que ainda não tem consumidor em `Config` continua entrando como constante citando no comentário a chave TOML de origem. O binário tem ícone próprio, entregue fora de fase: `app_icon.rs` decodifica um PNG embutido em runtime (crate `png`) e um `build.rs` com `winres` embute o `.ico` como recurso PE no Windows; `dirs` entrou junto, para o diretório home ser o `startup_directory`. Antes de mexer, leia [docs/arquitetura.md](docs/arquitetura.md) e os ADRs — o que está em código segue o que está escrito lá, incluindo os desvios anotados.
...
**Um** requisito do v1 ainda não tem desenho aprovado: aba restaurada sem shell iniciado (F5, seção 4.2 da especificação) — e é **recurso que não existe**, não desenho pendente na interface de hoje. Para ele vale o julgamento de quem implementa, ainda usando os tokens existentes. Os outros dois saíram: a cor de seleção de texto pelo ADR-0028 (o valor que o binário desenha é o deliberado) e os estilos `left-bar`/`outline` pelo [ADR-0032](docs/adr/0032-interface-do-v1-fechada.md), que os tirou de escopo em vez de lhes dar anatomia.
...
- **`porecatu-render` não tem primitiva de sombra nem de filtro de brilho — as duas são resolvidas fora dele, e é definitivo, não dívida.** `chrome::push_shadow` aproxima sombra com três `RoundedQuad` pretos empilhados (spread crescente, alfa decrescente); desde a F4 etapa 6 está na cápsula de grupo, na aba solta, no quadro do terminal, nos **cinco widgets de chrome** (aviso, diálogo, menu de contexto, tooltip, editor de grupo) e no **fantasma de arraste**. A mancha grande do CSS (`0 18px 44px`) não sai por essa técnica — aliaseia em anéis visíveis, por isso a aproximação em camadas. Hover por brilho é `chrome::brighten`, em CPU (multiplica canais e clampa), calculado fresco por frame a partir de `tab_bar::hit_test`: `1.18` na aba, `1.25` na pílula, sempre ligado (não condicionado a hover) no fantasma de arraste. **Essas duas eram a lista completa do que ainda mudava de pixel no v1** ([ADR-0032](docs/adr/0032-interface-do-v1-fechada.md) §2) — com as duas implementadas, a interface está fechada: mudar qualquer coisa das seções 1/2 da especificação visual exige ADR novo, falta de comportamento se registra, falta de aparência não existe.
...
### Descobertas na F4 — custaram horas, não repetir

- **Etapa que move constante pra `Config` e esquece de atualizar o script que verifica isso é bug silencioso até alguém rodar `verify-docs.py`.** A etapa 2 moveu ~30 constantes de `porecatu-ui` para `porecatu-config`, mas a tabela `VALORES` do script continuou apontando pros nomes antigos de `const` — 14 checagens reprovavam com "não existe — renomeada?" desde então, sem que `cargo test`/`clippy` acusassem nada (são scripts separados). `verify-docs.py` precisa rodar toda vez que uma constante muda de crate, não só quando o número muda.
- **`AnsiPalette::default()` devolve os valores de `normal`, não de `bright`.** Existe só para satisfazer o `#[serde(default)]` do container (`Colors::default()` é quem monta as duas metades certas, chamando `normal_default()`/`bright_default()` explicitamente). Usar `AnsiPalette::default()` como baseline de comparação para a metade **brilhante** de qualquer coisa (o merge de tema, aqui) compara contra o valor errado e nunca detecta "chave declarada fora do tema" nela — os testes do merge de tema pegaram isso; sem eles, todo tema teria a metade brilhante correta só por coincidência quando `bright.red` == `normal.red`.
- **Ação de config commitada numa etapa anterior pode não existir no catálogo fechado de uma etapa posterior.** Os defaults embutidos de `[keybindings]` (etapa 1) tinham `ctrl+shift+p`/`cmd+shift+p` → `"command.palette"`, uma ação `[v2]` fora do catálogo de 46 que a etapa 5 fechou. Um `Action::FromStr` correto rejeitaria o próprio default embutido do processo. `resolve()` ganhou um `debug_assert!` de que os defaults embutidos nunca produzem erro — é o que pegou isto antes de virar bug em produção.
- **Mouse sintético (`SetCursorPos`/`mouse_event`) atravessa a proteção de foco do Windows; teclado (`SendInput`) não.** Descoberto testando hover/sombra da etapa 6: mover o cursor e clicar (inclusive botão direito, abrindo menu de contexto) funciona em uma janela de processo em segundo plano, e permitiu verificar hover por brilho (medição de pixel, não só olho) e sombra sem pedir ao usuário. Teclado continua bloqueado — a dívida de verificação registrada desde a F1 é só da metade de teclado do gesto, não do mouse.
- **`filter: brightness()` em CPU é literalmente `canal * fator`, clampado — e o resultado não se vê a olho em tons já escuros.** `1.18` sobre `#252a33`-like muda uns 6-8 por canal; confirmar por captura de tela "por parecer diferente" é o tipo de verificação que erra por excesso de confiança. Medir o pixel (`Bitmap::GetPixel`) e conferir a razão contra o fator configurado é o teste que não engana.
- **Merge de tema por "diferente do default" é heurística, não rastreamento de origem.** Sem guardar se o usuário escreveu a chave ou se ela caiu no default do `#[serde(default)]`, "declarado fora do tema" só pode ser aproximado como "valor != default embutido" — que é exatamente o que fazer quando o usuário declara um valor **igual** ao default por coincidência (o tema "vence" nesse caso, mesmo a chave estando no arquivo). Aceito conscientemente: o efeito visual é idêntico de qualquer forma, já que o valor declarado é igual ao default.
