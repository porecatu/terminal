...
**Estado atual: F0, F1, F2 e a F3 implementadas — a F3 com uma exceção que a mantém aberta: o RF-2.21 (`group.next`/`group.prev`) não existe.** `cargo run` abre uma janela com abas de terminal funcionais e **grupos**: PTY, motor VT, render por GPU, teclado, mouse, seleção, clipboard, ciclo de vida de aba, overflow da barra, arraste, seleção múltipla, pílula e cápsula de cor, colapso, editor de grupo, menu de contexto de grupo, popover de destino, arraste entre grupos e do grupo inteiro, animação de reflui, tooltip, aviso e diálogo de confirmação — e `Ctrl+Shift+N` abre uma segunda janela. `porecatu-config` e `porecatu-session` ainda são stubs (só o cabeçalho SPDX); ganham corpo nas F4/F5. Enquanto `config` não existe, valor de aparência entra como constante citando no comentário a chave TOML de origem. Antes de mexer, leia [docs/arquitetura.md](docs/arquitetura.md) e os ADRs — o que está em código segue o que está escrito lá, incluindo os desvios anotados.

**O que falta para fechar a F3** está listado em [docs/roadmap.md](docs/roadmap.md): o RF-2.21 (o MRU está gravado em `Group::last_active`, falta a operação que anda de grupo em grupo), os atalhos das demais `group.*` — só `group.create` tem tecla —, e a verificação interativa, que segue pendente desde a F1.
...
Isso só se sustenta porque as cinco faces do design (**Iosevka Fixed** 400/500 no terminal, **Iosevka Aile** 400/500/600 no chrome) são **embutidas no binário** ([ADR-0025](docs/adr/0025-iosevka-no-lugar-da-ibm-plex.md)) — métrica de fonte diferente muda largura de célula e de aba. Elas são **recortadas** por `scripts/subset-fonts.py` (48 MB viram 2.1 MB); a OFL da Iosevka permite, por não ter Reserved Font Name. Rodar o script à mão ao subir a versão da fonte, e conferir a lista de faixas antes de assumir que um bloco novo desenha.
...
**A F3 abriu onze divergências novas entre desenho e binário**, todas na **seção 4.4** — que é onde a F4 vai cobrá-las, junto com as quatro da F2 (sem sombra, sem `brightness`, corpo de aviso em uma linha, auto-scroll por evento). Quatro delas mudaram a prosa da especificação, porque o comportamento novo passou a ser o alvo: a cápsula de cor cheia no lugar do tingimento de `.07` (§2.3, com o fundo da aba em alfa `.85` no §2.5), o botão de nova aba em **dois** lugares — zona fixa à direita e um por grupo (§2.2, §2.6) —, e o **fim da ordem de cedência** do overflow: nada encolhe, a trilha rola (§2.18, §2.4). As três primeiras foram pedido direto do usuário; a última, custo de medição de texto. Três outras entraram depois da fase, também a pedido do usuário: a **largura fixa da aba** (§2.5 — nada de refluir a trilha por causa de título), o **botão global criando aba desagrupada** (§2.6) e a **face de ícones** do ADR-0024, que é a única que não é escolha de desenho e sim ícone que não desenhava. Mais seis de acabamento, todas pedido do usuário depois de ver a barra em tela: o **nome do grupo na mesma fonte da aba** (§2.4 pedia 12px/500 contra 12.5px/400), o **tom de hover dos ícones promovido a tom de base** (`#e4e8ee`; o traço fino do Lucide some num cinza médio) com o **"+" do grupo indo para o escuro** (`#12151a`, único ícone que fica sobre a cápsula de cor), **sem o sublinhado de grupo** na base da aba (§2.5 — redundante desde que a cápsula virou cor cheia), **borda de aba em 2px** (1px não se lê contra a cápsula) e **aba de 34px numa trilha com 6px de respiro das bordas** (§2.2, §2.5 — a barra vai a 52px, acima dos 42 declarados). Mais uma: **aba solta é mais alta que aba agrupada** — dentro de um grupo ela cede `wrapper_padding` ao bloco, solta ocupa a caixa inteira do wrapper, e as duas alinham topo e base na barra. E, por fim, o **botão de nova aba global saiu** (era um segundo botão para a mesma ação a um palmo do "+" do grupo): a zona fixa à direita virou o bloco do **botão de configurações**, inerte até a F4, e a cor do "+" do grupo passou a depender do que está atrás dele. A cápsula de cor passou a ser desenhada **também com o grupo colapsado**, e os botões de ícone ganharam respiro horizontal (`icon_button_padding_x`). O "+" do grupo some quando ele colapsa, e um **"+" de aba solta** fecha a trilha quando o último grupo é explícito — sem ele, um workspace de um só grupo colapsado não teria gesto nenhum para criar aba fora de grupo.
...
- **`porecatu-render` não tem primitiva de sombra nem de filtro de brilho.** A especificação visual pede `filter: brightness()` no hover e sombra de popover nos widgets de chrome — quatro na F2, cinco desde o editor de grupo da F3; nenhum dos dois existe. Hover por brilho é resolvível em CPU dentro de `porecatu-ui` (multiplicar canais e clampar); sombra, não. Registrado na seção 4.4 da especificação.
