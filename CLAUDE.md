...
- **Trilha rolável e botão global não convivem.** Com a trilha rolando como um componente só, o botão de nova aba ao final dela sai de vista com muitas abas — botão inalcançável. Daí a zona fixa à direita (`right_zone_width`/`trilha_width`): a largura disponível para a trilha é a da barra **menos** essa zona, e é ela que `overflow_state` e o cálculo de arraste na borda devem usar.
