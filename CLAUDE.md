...
**Estado atual: F0, F1, F2 e F3 fechadas.** A verificação interativa das três fases é **dívida assumida**, não critério pendente: a proteção de foco do Windows bloqueia input sintético e não há ambiente Linux/macOS no fluxo, então elas fecharam com cobertura automatizada mais smoke test (ver a nota no topo do roadmap). `cargo run` abre uma janela com abas de terminal funcionais e **grupos**: PTY, motor VT, render por GPU, teclado, mouse, seleção, clipboard, ciclo de vida de aba, overflow da barra, arraste, seleção múltipla, pílula e cápsula de cor, colapso, editor de grupo, menu de contexto de grupo, popover de destino, arraste entre grupos e do grupo inteiro, animação de reflui, tooltip, aviso e diálogo de confirmação — e `Ctrl+Shift+N` abre uma segunda janela. Fora do macOS, a janela também perdeu a decoração nativa: drag region e botões de janela na própria barra de abas, resize por toda borda ([ADR-0027](docs/adr/0027-controles-de-janela-e-resize-proprios.md), pedido do usuário, fora de fase). **A F4 está em andamento**, na branch `feat/porecatu-ui-le-config`: a etapa 1 (`porecatu-config` nasce), a etapa 2 (`porecatu-ui` lê `Config` — chrome e barra de abas saem de `const`), a etapa 3 (terminal: fonte, cores, cursor, scrollback, seleção, clipboard, `[general] startup_directory` e `[shell]`) e a etapa 4 (hot reload: `notify` assistindo o diretório do arquivo de config, debounce de ~200ms, as três classes do [ADR-0030](docs/adr/0030-escopo-do-hot-reload.md) aplicadas por diff entre config antiga e nova, erro/chave-desconhecida na superfície de aviso do [ADR-0014](docs/adr/0014-superficie-de-aviso-e-dialogo.md)) já estão commitadas; a etapa 5 (`enum Action` em `porecatu-core`, gramática de tecla e resolução de `[keybindings]` em três níveis via [ADR-0029](docs/adr/0029-enum-de-acao-e-gramatica-de-tecla.md), `handle_tab_action_key` consultando o mapa resolvido em vez do `match` fixo) está em revisão. `porecatu-session` continua stub. As três decisões que destravaram a F4 já estão escritas ([ADR-0029](docs/adr/0029-enum-de-acao-e-gramatica-de-tecla.md), [ADR-0030](docs/adr/0030-escopo-do-hot-reload.md), [ADR-0031](docs/adr/0031-temas-nomeados.md)). Valor de aparência que ainda não tem consumidor em `Config` continua entrando como constante citando no comentário a chave TOML de origem. O binário tem ícone próprio, entregue fora de fase: `app_icon.rs` decodifica um PNG embutido em runtime (crate `png`) e um `build.rs` com `winres` embute o `.ico` como recurso PE no Windows; `dirs` entrou junto, para o diretório home ser o `startup_directory`. Antes de mexer, leia [docs/arquitetura.md](docs/arquitetura.md) e os ADRs — o que está em código segue o que está escrito lá, incluindo os desvios anotados.
